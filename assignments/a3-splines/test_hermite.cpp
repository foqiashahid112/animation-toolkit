...
int main()
{
   ...
   InterpolatorHermite hermite2;
   hermite2.setClamped(false);
   hermite2.computeControlPoints(keys);

   // todo: print the control points in hemite
   // verify they match the example from class!
   for (int i = 0; i < hermite2.getNumControlPoints(); i++)
   {
      std::cout << "control point: " << to_string(hermite2.getControlPoint(i)) << std::endl;
   }

   value = hermite2.interpolate(0, 0.1);
   std::cout << "(natural) interpolate(0, 0) = " << to_string(value) << std::endl;

   value = hermite2.interpolate(3, 0.9);
   std::cout << "(natural) interpolate(3, 1) = " << to_string(value) << std::endl;

   value = hermite2.interpolate(2, 0.5);
